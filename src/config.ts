...
export class BotConfig {
	constructor(
		...
		private overrides: PartialConfiguration = {}
	) {
		dotenv.config();
	}
	...
	public setConfigOverrides(overrides: PartialConfiguration) {
		this.overrides = overrides;
	}
	public clearConfigOverrides() {
		this.overrides = {};
	}

	private getConfigValue(key: keyof ServiceConfiguration): string {
		const value = this.overrides[key] ?? this.config[key] ?? this.defaults[key];
		if (value === undefined) {
